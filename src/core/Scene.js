 ...
 * @property {{curveEndpoint: number, interactionMerging: number, interactionNormal: number, forwardDistance: number, rayPowerCutoff: number, rayPowerCutoffMode: string}} numericalTolerances - Numerical thresholds used by primitive-based simulation engines. Distance values are relative to the scene length scale, `interactionNormal` is an angle in radians, and `rayPowerCutoff` is a dimensionless power. Zero selects only engine-derived geometric tolerance or disables the power cutoff. `rayPowerCutoffMode` selects stable power-weighted sampling or deterministic truncation. Primitive simulation in default color mode uses stable sampling with a cutoff of at least 0.01 for consistent CPU/WebGPU behavior.
...
class Scene {
  ...
  static nestedProperties = ['theme', 'numericalTolerances'];
