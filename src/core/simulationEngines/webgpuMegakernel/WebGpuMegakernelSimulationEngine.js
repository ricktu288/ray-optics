 ...
 */
class WebGpuMegakernelSimulationRun {
 ...
 */
class WebGpuMegakernelSimulationEngine {
  constructor({
    device = null,
    output = null,
  ...
  async createRun(options = {}) {
    ...
    const run = new WebGpuMegakernelSimulationRun(this, options);
    if (!isCurrent()) {
      run.cancel();
      return run;
    }
  ...
  async initialize() {
    ...
    if (!this.deviceSource || !this.output) {
      throw new Error(
        'WebGpuMegakernelSimulationEngine requires a WebGPU device and output. ' +
    ...
    this.executionMode = 'webgpu-megakernel-raster-atomic';
...
export default WebGpuMegakernelSimulationEngine;
