...
locales["zh-CN"] = {
  ...
  "help_popover": {
    "message": "<div style=\"font-weight: normal; font-size: 10pt;\"><b>提示泡泡</b><p>按此钮以切换是否于其他按钮显示提示泡泡。</p><b>限制拖曳行为</b><p>拖曳物件时，按住 Shift 以限制于水平或垂直移动；按住 Ctrl 以复制物件。建立线型物件或拖曳其端点时，按住 Shift 以锁定角度或吸附至 0°、45°、90°；按住 Ctrl 以对于中心点旋转或缩放。</p><b>群组、旋转及缩放物件</b><p>按住 Ctrl，点击多个控制点（如端点），并点击空白处以将这些点群组并建立控制杆。拖曳控制杆以平移群组；Shift+拖曳以限制于水平或垂直移动；Ctrl+拖曳以旋转；Ctrl+Shift+拖曳以缩放。移动「x」以设定旋转或缩放中心。按 Delete 以移除控制杆。</p><b>直接编辑坐标</b><p>对控制点（如端点）点两下或按右键，输入 (x,y) 坐标（单位为像素），然后按 Enter。 </p><b>键盘快速键</b><p>当一物件在被选取的状态（被点击）时，按方向键以移动；Delete 以删除；Ctrl+D 以复制。 </div>"
  }
}
